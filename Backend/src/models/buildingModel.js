...
const BuildingModel = {
  ...
  async create(buildingData) {
    ...
    // Get campus internal ID - support both campus_uid and campusId
    const campusUid = buildingData.campus_uid || buildingData.campusId;
    const [campus] = await db.query('SELECT id FROM campuses WHERE uid = ?', [campusUid]);
